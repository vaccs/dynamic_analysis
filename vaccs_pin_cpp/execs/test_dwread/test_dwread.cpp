...
TEST_CASE("test_cu_table_record") {
	...
	cu_record *curec;
	REQUIRE((curec = (cu_record*)rec_factory.make_symbol_table_record(CU_RECORD)) != nullptr);

	dwarf::line_table lt;
	REQUIRE((curec->add_high_pc(0x12345L)
			->add_low_pc(0x98765L)
			->add_type_table((type_table*)0x34567L)
			->add_var_table((var_table*)0x87654L)) != nullptr);

	REQUIRE(curec->get_high_pc() == 0x12345L);
	REQUIRE(curec->get_low_pc() == 0x98765L);
	REQUIRE(curec->get_type_table() == (type_table*)0x34567L);
	REQUIRE(curec->get_var_table() == (var_table*)0x87654L);

	cu_table *cutab;
	REQUIRE((cutab = (cu_table*)tab_factory.make_symbol_table(CU_TABLE)) != nullptr);
...
TEST_CASE("test_type_table") {
	...
	type_record *trec;
	REQUIRE((trec = (type_record*)rfac.make_symbol_table_record(TYPE_RECORD)) != nullptr);
	...
	type_table *tt;
	REQUIRE((tt = (type_table*)tfac.make_symbol_table(TYPE_TABLE)) != nullptr);
...
TEST_CASE("test_var_table") {
	...
	var_record *vrec;
	REQUIRE((vrec = (var_record*)rfac.make_symbol_table_record(VAR_RECORD)) != nullptr);

	var_table *vt;
	REQUIRE((vt = (var_table*)tfac.make_symbol_table(VAR_TABLE)) != nullptr);
