 ...
 * Constructor
 */
sd_set_record::sd_set_record() :
		vaccs_record(VACCS_SD_SET) {
	event_num = -1;
	c_line_num = -1;
	c_file_name = NULL;
	scope = NULL;
...
/**
 * Write an set record to the analysis file
 *
 * @param fp a file pointer for the analysis file
 */
void sd_set_record::write(NATIVE_FD fd) {
	vaccs_id_t id = VACCS_SD_SET;
...
/**
 * Read an set record from the analysis file
 *
 * @param fp a file pointer for the analysis file
 * @return an set record
 */
vaccs_record *sd_set_record::read(NATIVE_FD fd) {
