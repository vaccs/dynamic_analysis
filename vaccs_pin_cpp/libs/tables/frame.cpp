...
extern void exit_analysis(); // defined in PAS/pas_analysis.cpp
 ...
 */
bool frame::in_user_code(Generic ip)
{
...
runtime_stack::push(string name, Generic ip, CONTEXT *ctx)
{
	...
	frame *new_frame = (new frame())->add_name(name)->add_context(ctx)->add_start_pc(fvrec->get_start_pc())->add_high_pc(fvrec->get_high_pc());
										  ...
										  string prefix)
{
	...
	{
		int index = (addr - vrec->get_base_address(fr->get_context())) / element_size;
			...
			else
			{
				...
				if (fr->get_is_first_access() || new_value != frec->get_value())
				{
					...
					if (trec->get_is_pointer())
					{
						...
						DEBUGL(LOG("points_to value converted from " + frec->get_points_to_value() + " to " + hexstr(pointer_val)));
	...
	for (list<frame_record *>::iterator it2 = fr->begin(); it2 != fr->end(); it2++)
	{
		...
		if (trec->get_is_pointer())
		{
			...
			else
			{
				...
				{
					DEBUGL(LOG(
						"Non-character Pointer variable: " + frec->get_variable_name() + " points to a valid memory location " + hexstr(ptr_addr) + " of type " + *trec->get_name() + "\n"));
					new_value = vrec->read_value(ttab, trec, ptr_addr, fr->get_context());
				}
				else if (trec->get_name()->compare("char*") != 0)
				{ // character strings are handled specially
					DEBUGL(LOG(
						"Non-character Pointer variable: " + frec->get_variable_name() + " points to a valid memory location " + hexstr(ptr_addr) + " of type " + *trec->get_name() + "\n"));
				...
				else
				{
					...
					DEBUGL(LOG(
						"Character Pointer variable: " + frec->get_variable_name() + " points to a valid memory location " + hexstr(ptr_addr) + " of type " + *btrec->get_name() + "\n"));
			...
			{
				DEBUGL(LOG("Found new points_to value: " + new_value + " for " + frec->get_variable_name() + " old points_to value = " + frec->get_points_to_value() + "\n"));
